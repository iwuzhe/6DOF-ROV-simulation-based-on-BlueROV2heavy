...
class ROVConfig:
    def __init__(self, num_envs=1, device='cuda'):
        ...
        # === 物理参数 (标称值) ===
        # 注意：这里不再进行随机化，只定义标准参数
        self.base_m = 14.5
        self.m = self.base_m * torch.ones(num_envs, 1, device=device)

        self.g = 9.81
        self.W = self.m * self.g  # 重力
        self.B = 143.0  # 浮力 (标称值)
        ...
        # 线性阻尼 (标称值)
        self.D_lin = torch.tensor([4.03, 6.22, 5.18, 0.07, 0.07, 0.07], device=device).repeat(num_envs, 1)
        ...
        # RL Scale
        self.rl_scale = torch.tensor([15.0, 15.0, 15.0, 8.5, 8.5, 8.5], device=device)
