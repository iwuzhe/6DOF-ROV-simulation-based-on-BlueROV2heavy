...
class ParallelESO:
    def __init__(self, config, num_envs, device):
        ...
        # ESO 带宽设置
        w0 = torch.tensor([3.0, 3.0, 3.0, 6.0, 6.0, 6.0], device=device).repeat(num_envs, 1)
    ...
    def update(self, state_meas, last_thrust_cmd_newton):
        ...
        # [修复] 质量矩阵计算：解决广播维度错误
        # M_mass: [B, 1] + [3] -> [B, 3] (自动广播)
        M_mass = self.cfg.m + self.cfg.M_added[0:3]

        # M_inertia: [B, 1] + scalar -> [B, 1]
        # 使用切片 [:, 0:1] 而不是索引 [:, 0] 来保持维度，方便拼接
        M_Ixx = self.cfg.I[:, 0:1] + self.cfg.M_added[3]
        M_Iyy = self.cfg.I[:, 1:2] + self.cfg.M_added[4]
        M_Izz = self.cfg.I[:, 2:3] + self.cfg.M_added[5]

        # 拼接为 [B, 6]
        M_diag = torch.cat([M_mass, M_Ixx, M_Iyy, M_Izz], dim=1)
