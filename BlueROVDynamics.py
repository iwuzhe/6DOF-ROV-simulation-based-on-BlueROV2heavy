...
class BlueROVDynamics:
    ...
    def dynamics_step(self, state, thrust_cmd_pwm, ocean_current_force=None):
        ...
        # [修复] 兼容 M_added 是 1D (固定值) 或 2D (随机化 batch) 的情况
        if self.cfg.M_added.ndim == 1:
            # 这里的索引 [0], [1]... 取的是标量
        ...
        # --- 2. 科里奥利矩阵 (包含附加质量影响) ---
        # 注意：这里的 a1~a6 系数也需要根据是否 batch 进行适配
        ...
        # --- 3. 阻尼与恢复力 ---
        # D_lin 和 D_quad 在 Config 中通常已经是 repeat 过的或者 broadcast 兼容的，无需修改
        ...
        # 姿态角缠绕处理 (Attitude Wrapping) [-pi, pi]
